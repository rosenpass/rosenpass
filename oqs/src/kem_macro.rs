...
macro_rules! oqs_kem {
    ($name:ident) => {
        ::paste::paste!{
            ...
            mod [< $name:snake >] {
                use rosenpass_cipher_traits::{Kem, KemError};
                ...
                impl Kem<SK_LEN, PK_LEN, CT_LEN, SHK_LEN> for [< $name:camel >] {
                    fn keygen(sk: &mut [u8; SK_LEN], pk: &mut [u8; PK_LEN]) -> Result<(), KemError> {
                    ...
                    fn encaps(shk: &mut [u8; SHK_LEN], ct: &mut [u8; CT_LEN], pk: &[u8; PK_LEN]) -> Result<(), KemError> {
                        assert_eq!(shk.len(), SHK_LEN);
                    ...
                    fn decaps(shk: &mut [u8; SHK_LEN], sk: &[u8; SK_LEN], ct: &[u8; CT_LEN]) -> Result<(), KemError> {
                        assert_eq!(shk.len(), SHK_LEN);
                        assert_eq!(sk.len(), SK_LEN);
                        assert_eq!(ct.len(), CT_LEN);
                        unsafe {
