...
impl CliArgs {
    ...
    pub fn apply_to_config(&self, _cfg: &mut cfg::AppServer) -> anyhow::Result<()> {
        #[cfg(feature = "experiment_api")]
    ...
    pub fn run(
        ...
        match &self.command {
            ...
            Some(GenKeys {
                ...
                let (pkf, skf) = match (config_file, public_key, secret_key) {
                    (Some(config_file), _, _) => {
                        ...
                        let config = cfg::AppServer::load(config_file)?;
                        let keypair = config
                            .keypair
            ...
            Some(ExchangeConfig { config_file }) => {
                ...
                let mut config = cfg::AppServer::load(config_file)?;
                config.validate()?;
            ...
            Some(Exchange {
                ...
                let mut config = cfg::AppServer::parse_args(args)?;
