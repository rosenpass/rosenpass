...
impl<Variables: Borrow<ShmPipeVariables>> ShmPipeCore<Variables> {
    /// Constructor
    ///
    /// Builds the pipe core from the synchronization `variables` and the
    /// shared memory segment `buf` holding the data plane.
    ///
    /// # Requirements
    ///
    /// If the synchronization `variables` are shared with another process –
    /// e.g. placed inside a shared memory segment, rather than kept in
    /// process-local memory behind an [Arc](std::sync::Arc) as
    /// [super::shm_pipe()] does – their operations must be genuinely atomic
    /// read-modify-write accesses that are visible to all processes sharing
    /// the memory the variables live in; an implementation merely emulating
    /// atomic access (e.g. via a [std::cell::Cell] or a process-local
    /// mutex) would make concurrent access from another process a data
    /// race. The [AtomicU64] used by this implementation is genuinely
    /// atomic on all platforms supported by this crate.
