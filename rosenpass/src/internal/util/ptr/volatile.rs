...
//!
//! See the documentation of [`read_volatile`](pointer#method.read_volatile)/[`write_volatile`](pointer#method.write_volatile).
...
pub trait ReadMemVolatile<T> {
    /// Read from a memory location using
    /// [`read_volatile`](pointer#method.read_volatile) in a loop and store the
    /// results in an array
    ///
    /// # Safety
    ///
    /// See [`read_volatile`](pointer#method.read_volatile).
...
pub trait WriteMemVolatile<T>: ReadMemVolatile<T> {
    /// Write to a memory location using
    /// [`write_volatile`](pointer#method.write_volatile) in a loop
    ///
    /// # Safety
    ///
    /// See [`write_volatile`](pointer#method.write_volatile).
