...
use crate::internal::util::{
    convert::IntoTypeExt,
    functional::MutatingExt,
    int::{
        modular::Modulus,
        u64uint::{MAX_U64_IN_USIZE, TruncateIntoU64USize, U64USize, U64USizeConversionError},
    },
...
/// We need a default for [tinyvec::ArrayVec] to work
...
pub enum ScheduledOperationCreationError {
    /// Cause of this error
    #[error("Could not create ScheduledOperations (list of ScheduledOperation); cumulative operation length exceeds the maximum: {:?} (maximum is {})", .0, MAX_U64_IN_USIZE)]
    OperationTooLong(#[from] U64USizeConversionError<u128>),
...
/// The type returned by [RingBufferScheduler::schedule_contiguous_operations()] and sister
/// functions.
...
/// represented by [RingBufferScheduler::schedule_contiguous_operations()] and sister methods
...
where
    ...
    for<'a> &'a mut Cont: IntoIterator<Item = &'a mut ScheduledOperation>,
{
    /// Ensure this ScheduledOperations is in a sound state
    fn demand_soundness(&self) -> Result<(), ScheduledOperationCreationError> {
        ...
        self.try_cumulative_operation_length()?;
    ...
    ///
    /// Calculates the [Self::cumulative_operation_length()], but returns an error
    /// if the length overflows a U64USize
    fn try_cumulative_operation_length(&self) -> Result<U64USize, U64USizeConversionError<u128>> {
        self.cont
            .borrow()
            .into_iter()
    ...
    ///
    /// This would be the total number of bytes written/read if all of the operations were
    /// applied.
    ///
    /// # Examples
    ///
    ...
    pub fn cumulative_operation_length(&self) -> U64USize {
        // No panic: We test for overlong cumulative operation lengths on creation
        self.try_cumulative_operation_length().expect(
            "This is a bug: Cumulative operation length overflows despite the check upon creation",
        )
    }

    /// Sum of the operation lengths before an operation, for each operation.
    ///
    /// In an operation between a linear buffer and a ring buffer, this represents the offset in
...
/// Represents the information of whether two values are the same or different
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
#[derive(thiserror::Error, Debug)]
#[error(
    "Expected ring buffer scheduler diff to only contain a change in the number of items written, but the actual diff is: {diff:?}"
)]
...
///   about how much data has been read
...
/// the functions scheduling operations (e.g. [Self::schedule_contiguous_operations]) never schedule
...
///
/// [Self] aggressively uses [U64USize] in various APIs, to make the size requirements and behavior
...
/// ```
///
/// They are represented as u64 internally.
///
...
/// ```
///
/// This representation also makes sure we can handle all allowed buffer sizes with ease.
///
...
/// Block 2 becomes Block 0 which is zero-sized. Take this example:
///
/// ```txt
/// buf_len = 100
/// items_read = 120
...
/// buf_len = 100
/// items_read = 120
/// items_written = 180 + 20 = 200 ≡ 0 (mod 2 * buf_len)
///
/// read_off  = items_read    % buf_len = 120 % 100 = 20
/// write_off = items_written % buf_len =   0 % 100 = 0
///
/// layout = DataFreeData
///
/// |       Block 0       |       Block 1        |       Block 2    |
/// |        Data         |        Free          |        Data      |
/// |      (0 items)      |      (20 items)      |      (80 items)  |
///
...
pub enum RingBufferNewError {
    ...
    BufferTooLarge(u64),
    /// The buffer size is zero. Zero-sized ring buffers are not usable: They can not
    /// hold any items and operations such as [RingBufferScheduler::layout] are undefined
    /// for them.
    #[error("Requested buffer size (0) is not supported: Zero-sized ring buffers are not usable")]
    ZeroBufferLength,
}
...
pub enum RingBufferFromCountersError {
    ...
    #[error("For a ring buffer of length {buf_len}, \
        data_avail must be <= {buf_len}, but the actual value is {data_avail}: \
        data_avail = items_written - items_read = {items_written} - {items_read} \
        = {data_avail} (mod 2 * buf_len = {bl2})",
        // Use the same construction as [RingBufferScheduler::buf_double_modulus]:
        // `Modulus::double` represents the modulus 2^64 exactly (full range), which
        // occurs at buf_len == MAX_BUF_LEN; `wrapping_mul(2) + new_or_panic` would
        // panic there (an error path crashing while reporting the error). The
        // full-range fallback also keeps hand-constructed errors (buf_len == 0 or
        ...
        // u128 so that 2 * buf_len is displayed truthfully even at buf_len ==
        // MAX_BUF_LEN, where the value does not fit into a u64
        bl2 = (*buf_len as u128) * 2)]
...
impl RingBufferScheduler {
    ...
    /// Ensure this ring buffer is in a sound state
    fn demand_soundness(&self) -> Result<(), RingBufferFromCountersError> {
        ...
        // Zero-sized ring buffers are not usable: They can not hold any items and
        // [Self::fill_state] would consider them both empty and full, so [Self::layout],
        // the `schedule_*` functions and the [Debug](std::fmt::Debug) impl would panic
        // on them. A value the constructor blesses must be usable, so zero-sized buffers
        // are rejected here — as an error, not a panic: In shared-memory use the
        // counters are writable by a remote peer.
        (buf_len != 0)
            .then_some(())
            .ok_or(RingBufferNewError::ZeroBufferLength)?;
    ...
    /// # Panic
    ///
    /// Panics if buf_len > [Self::MAX_BUF_LEN] or if buf_len == 0
    ...
    ///
    /// Returns an error if the write is longer than what the buffer can currently support.
    #[allow(clippy::double_must_use)]
    #[must_use]
    ...
    /// # Panic
    ///
    /// Panics if the write is longer than currently possible according to [Self::space_avail()]
    #[must_use]
    ...
    /// Used to register an operation generated using [Self::schedule_next_write]/[Self::schedule_next_read]/[Self::schedule_reads]/[Self::schedule_writes],
    ...
    /// Number of total items read from the ring buffer (mod 2 * buf_len); this is exactly the
    /// value passed to [Self::from_counters]
    pub fn items_read(&self) -> u64 {
        self.items_read
    }

    /// Number of total items written to the ring buffer (mod 2 * buf_len); this is exactly the
    /// value passed to [Self::from_counters]
    ...
    /// Length of the ring buffer; this is exactly the value passed to
    /// [Self::new]
    ...
    /// [Self::buf_modulus()].[double()](Modulus::double).[unwrap()](Option::unwrap); we know this
    ...
    /// Calls [Self::data_avail] for reads or [Self::space_avail] for writes
    ...
    ///     if items_written != items_read then items_written - items_read (mod 2 * buf_len)
    /// ```
    ///
    ...
    /// ```
    ///
    /// Unfortunately, this method breaks when the underlying u64 value overflows.
    #[rustfmt::skip]
    ...
    /// Whether the ring buffer is not empty
    pub fn is_nonempty(&self) -> bool {
        !self.is_empty()
    }

    /// Whether the ring buffer is not full
    ...
    /// Access metadata about all Blocks in the ring buffer
    ///
    /// See [Self] for more information on the ring buffer memory layout
    ...
    /// the operation given.
    pub fn schedule_next_contiguous_operation<Size: TruncateIntoU64USize>(
        &self,
        op: OperationType,
        max_len: Size,
    ...
    pub fn schedule_next_read<Size: TruncateIntoU64USize>(
        ...
        self.schedule_next_contiguous_operation(OperationType::Read, dst_len)
    }
    ...
    ///
    /// The result takes the source length into account and can be immediately used
    /// for writes from a contiguous slice of memory into the ring buffer.
    ///
    /// Passing arbitrarily large source sizes is valid; passing [usize]::MAX or [u64]::MAX as the
    /// source length is explicitly supported and will just yield the maximum possible
    /// contiguous write supported by the ring buffer.
    ///
    /// Note that this function will never schedule operations of a length that can not be represented
    /// in both a u64 and a usize. For 64 bit systems, this is not a limitation in practice. For 32
    /// bit systems it means that scheduled operations are restricted to around 4GB of data
    ...
    pub fn schedule_next_write<Size: TruncateIntoU64USize>(
        ...
        self.schedule_next_contiguous_operation(OperationType::Write, src_len)
    }
    ...
    pub fn schedule_contiguous_operations<Size: TruncateIntoU64USize>(
        &self,
        op_type: OperationType,
        max_len: Size,
    ) -> ScheduledOperations {
        let max_len = max_len.truncate_to_u64usize();
        let op1 = self.schedule_next_contiguous_operation(op_type, max_len);
        let op2 = op1.and_then(|op1| {
            self.register_operation(&op1)
                .schedule_next_contiguous_operation(op_type, max_len.u64() - op1.len.u64())
        });

        let cont = [op1, op2].iter().filter_map(|maybe_op| *maybe_op).collect();

        // Won't panic, since we truncated max_len
    ...
    pub fn schedule_reads<Size: TruncateIntoU64USize>(&self, dst_len: Size) -> ScheduledOperations {
        self.schedule_contiguous_operations(OperationType::Read, dst_len)
    }
    ...
    pub fn schedule_writes<Size: TruncateIntoU64USize>(
        ...
        self.schedule_contiguous_operations(OperationType::Write, src_len)
    }
}
...
mod tests {
    ...
    fn cumulative_operation_length_overflow_is_an_error() {
        use AbstractScheduledOperations as Ops;
        use OperationType::Read as R;
        use ScheduledOperationCreationError as E;

        let op = |len: u64| ScheduledOperation::new(R, 0, U64USize::new_or_panic(len));

        // The boundary case, a cumulative length of exactly u64::MAX, still works
        assert!(Ops::try_new(vec![op(MAX_U64_IN_USIZE - 1), op(1)]).is_ok());

        // One item more and the cumulative length overflows u64; this must be an error,
        // not a panic (debug builds) or a wrapped sum (release builds)
        assert!(matches!(
            Ops::try_new(vec![op(MAX_U64_IN_USIZE - 1), op(2)]),
            Err(E::OperationTooLong(_))
        ));
        assert!(matches!(
            Ops::try_new(vec![op(MAX_U64_IN_USIZE), op(1)]),
            Err(E::OperationTooLong(_))
        ));
    }

    /// Formatting [RingBufferFromCountersError::DataAvailOutOfBounds] at
    /// `buf_len == MAX_BUF_LEN` (2^63) must not panic: `2 * buf_len` does not fit into
    /// a u64 there, so the `Display` implementation must use the full-range modulus
    /// trick ([Modulus::double]) instead of `wrapping_mul` + `new_or_panic`.
    #[test]
    fn data_avail_out_of_bounds_display_works_at_max_buf_len() {
        // 0 - 1 mod 2^64 = u64::MAX > MAX_BUF_LEN, so this constructs the error
        // through the real soundness-check path
        let err = RingBufferScheduler::try_from_counters(RingBufferScheduler::MAX_BUF_LEN, 0, 1)
            .unwrap_err();
        assert!(matches!(
            err,
            RingBufferFromCountersError::DataAvailOutOfBounds { .. }
        ));

        // Must not panic while reporting the error
        let msg = format!("{err}");
        assert!(msg.contains("18446744073709551615")); // data_avail = 0 - 1 (mod 2^64)
        assert!(msg.contains("18446744073709551616")); // 2 * buf_len = 2^64
    }

    /// [RingBufferScheduler::try_from_counters] is documented as the non-panicking
    /// constructor: it must reject `buf_len == 0` with nonzero counters by
    /// returning an error, never by panicking. In shared-memory use the
    /// counters are writable by a remote peer, so a panic here would be a
    /// remotely triggerable denial of service.
    #[test]
    fn try_from_counters_rejects_nonzero_counters_for_zero_sized_buffer() {
        for (items_written, items_read) in [(1, 0), (0, 1), (1, 1), (u64::MAX, u64::MAX)] {
            let res = RingBufferScheduler::try_from_counters(0, items_written, items_read);
            assert!(
                matches!(
                    res,
                    Err(RingBufferFromCountersError::RingBufferNewError(
                        RingBufferNewError::ZeroBufferLength
                    ))
                ),
                "try_from_counters(0, {items_written}, {items_read}) = {res:?}"
            );
        }

        // The all-zero representation of a zero-sized buffer is rejected;
        // zero-sized schedulers are not usable and are refused at construction
        assert!(matches!(
            RingBufferScheduler::try_from_counters(0, 0, 0),
            Err(RingBufferFromCountersError::RingBufferNewError(
                RingBufferNewError::ZeroBufferLength
            ))
        ));
    ...
    /// Zero-sized schedulers are not usable ([RingBufferScheduler::fill_state] would
    /// consider them both empty and full; [RingBufferScheduler::layout], the
    /// `schedule_*` functions and the [Debug](std::fmt::Debug) impl would panic on
    /// them), so construction refuses them. A value the constructor blesses must be
    /// usable.
    #[test]
    fn zero_sized_schedulers_are_rejected_at_construction() {
        assert_eq!(
            RingBufferScheduler::try_new(0),
            Err(RingBufferNewError::ZeroBufferLength)
        );
        assert!(matches!(
            RingBufferScheduler::try_from_counters(0, 0, 0),
            Err(RingBufferFromCountersError::RingBufferNewError(
    ...
    /// The smallest valid scheduler (buf_len == 1) must still be constructible and fully
    /// usable: [RingBufferScheduler::fill_state], [RingBufferScheduler::layout], the
    /// `schedule_*` functions and the [Debug](std::fmt::Debug) impl must all work on it
    #[test]
    fn smallest_valid_scheduler_is_usable() {
        let sched = RingBufferScheduler::try_new(1).unwrap();
        assert_eq!(sched.buf_len(), 1);
        assert_eq!(sched.fill_state(), BufferFillState::Empty);
        assert_eq!(sched.layout(), BufferLayout::FreeDataFree);
        // Formatting must not panic
        let _ = format!("{sched:?}");

        // A one-item write fills the buffer completely; scheduling still works
        let op = sched.schedule_next_write(1u8).unwrap();
        let sched = sched.register_operation(&op);
        assert_eq!(sched.fill_state(), BufferFillState::Full);
        // Formatting must not panic
        let _ = format!("{sched:?}");
    }
}
