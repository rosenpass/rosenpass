...
fn key_exchange_with_app_server_v02() -> anyhow::Result<()> {
    key_exchange_with_app_server(cfg::ProtocolVersion::V02)
}

#[test]
#[cfg_attr(miri, ignore)] // unsupported operation: can't call foreign function `mprotect` on OS `linux`
fn key_exchange_with_app_server_v03() -> anyhow::Result<()> {
    key_exchange_with_app_server(cfg::ProtocolVersion::V03)
}

fn key_exchange_with_app_server(protocol_version: cfg::ProtocolVersion) -> anyhow::Result<()> {
        ...
        thread::spawn(move || {
            run(move || -> anyhow::Result<()> {
                ...
                srv.app_srv.add_peer(
                    ...
                    hostname,
                    protocol_version,
                    protocol::OskDomainSeparator::default(),
                )?;
...
impl TestServer {
    fn new(termination_queue: mpsc::Receiver<()>) -> anyhow::Result<Self> {
        ...
        let verbosity = cfg::Verbosity::Verbose;
