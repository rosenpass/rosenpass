...
/// A convenience type for working with buffers and extracting [`zerocopy::Ref`] references.
///
...
impl<B: ByteSlice, T> RefMaker<B, T> {
    ...
    /// Splits the internal buffer into a `RefMaker` containing a buffer with exactly `size_of::<T>()` bytes
    /// and the remaining tail of the previous internal buffer.
    ///
    /// # Errors
    ///
    ...
    /// Splits the buffer into two `RefMaker`s, with the first containing the first `size_of::<T>()` bytes
    /// and the second containing the remaining tail buffer.
    ///
    /// # Errors
    ///
    /// Returns an error if the buffer is undersized.
    ...
    /// Splits the buffer into a `RefMaker` containing the last `size_of::<T>()` bytes as [RefMaker]
    /// and the preceding bytes as a buffer.
    ///
    /// # Errors
    ///
    /// Returns an error if the buffer is undersized.
    ...
    /// Splits the buffer into two `RefMaker`s, with the second containing the last `size_of::<T>()` bytes,
    /// and the first containing the remaining preceding bytes.
    ///
    /// # Errors
    ///
    /// Returns an error if the buffer is undersized.
