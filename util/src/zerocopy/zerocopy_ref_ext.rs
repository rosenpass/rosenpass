//! Extension traits for converting `Ref<B, T>` into references backed by standard slices.
...
///
/// Note: This trait is implemented to [Ref](zerocopy::Ref) of byte slices (`&[u8]`).
...
///
/// Note: this trait is implemented to [Ref](zerocopy::Ref) of mutable byte slices (`&mut [u8]`).
pub trait ZerocopyEmancipateMutExt<B, T> {
    /// Converts this reference into a mutable reference backed by a plain mutable byte slice.
    ///
    /// # Example
    ///
    /// ```
