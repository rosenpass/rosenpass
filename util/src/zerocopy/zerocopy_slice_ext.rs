//! Extension traits for parsing slices into [`zerocopy::Ref`] values using the [`RefMaker`] abstraction.
...
///
/// This trait adds methods for creating [`Ref`](zerocopy::Ref) references from slices
/// by using the [`RefMaker`] type internally.
...
/// Extension trait for zero-copy parsing of mutable slices with zeroization capabilities.
///
...
pub trait ZerocopyMutSliceExt: ZerocopySliceExt + Sized + ByteSliceMut {
    ...
    ///
    /// Zeroizes the first `target_size()` bytes of the slice, then returns a `Ref`.
    ///
    /// # Errors
    ///
    /// Returns an error if the slice is too small.
    ...
    ///
    /// Zeroizes the last `target_size()` bytes of the slice, then returns a `Ref`.
    ///
    /// # Errors
    ///
    /// Returns an error if the slice is too small.
